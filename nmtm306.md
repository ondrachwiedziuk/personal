...
Z Thalétovy kružnice plyne, že $|KX|^2 = |MX| \cdot |XN|$. Jelikož je řez rovnoběžný s přímkou $AG$, tak $|XN| = 
|HG|$. Z podobnosti trojuhelníků $VMX$ a $VBH$ plyne $ \frac{|MX|}{|BH|} = \frac{|VX|}{|VH|} $. Proto:

$$ |KX|^2 = |MX|\cdot|XN| = \frac{|VX|\cdot|BH|}{|VH|}\cdot|HG| = |VX|\cdot\frac{|BH|\cdot|HG|}{|VH|} $$

Nyní konstantu $\frac{|BH|\cdot|HG|}{|VH|} $ označíme jako $p$ a také $|KX| = y$ a $|VX| = x$. Tedy dostaneme hledaný vztah $y^2 = px$.
